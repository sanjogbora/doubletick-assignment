...
import { Sparkle, Sparkles, Calendar, Check, Clock, AlertCircle, FileText, ChevronDown, Info, Minus, ThumbsUp, ThumbsDown } from 'lucide-react';
...
const AIActionPanel: React.FC<AIActionPanelProps> = ({
    ...
    if (minimized) {
        return (
            <div
                className={`flex items-center gap-2 bg-white/90 backdrop-blur-sm border border-purple-100 rounded-full px-3 py-1.5 cursor-pointer hover:bg-purple-50 transition-all ${className}`}
...
const SuggestionCard: React.FC<{
    ...
    const handleAccept = () => {
        ...
        // Only show "Action completed" immediately for direct actions that don't require a modal
            ...
            <div className="flex gap-3 items-start">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between gap-2 mt-2">
                        <p className="text-xs text-gray-600 leading-tight truncate flex-1 min-w-0" title={suggestion.description}>{suggestion.description}</p>

                        {/* Inline Actions */}
                        <div className="flex items-center gap-1.5 shrink-0">
                            <button
                                onClick={() => onDismiss(suggestion.id)}
                                className="text-[10px] text-gray-400 hover:text-gray-600 px-1.5 py-0.5 rounded hover:bg-gray-100 transition-all"
                            >
                                Dismiss
                            </button>
                            <button
                                onClick={handleAccept}
                                className={`text-[10px] font-medium px-2 py-0.5 rounded shadow-sm flex items-center gap-1 transition-all active:scale-95 ${suggestion.actionType === ActionType.SCHEDULE_FOLLOWUP
